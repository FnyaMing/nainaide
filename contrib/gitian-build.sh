...
DEFAULT_NAINAIDE_SIGS=${NAINAIDE_SIGS:-'nainaide.sigs'}
DEFAULT_GITIAN_REPO='https://github.com/devrandom/gitian-builder'
DEFAULT_GBUILD_FLAGS=''
DEFAULT_SIGS_REPO='https://github.com/FnyaMing/nainaide.sigs'
...
The signature will be saved in '${DEFAULT_NAINAIDE_SIGS}/'. An alternative output directory
for signatures can be supplied via the environment variable \$NAINAIDE_SIGS.
...
f_build() {
  ...
  bin/gbuild --commit nainaide="$g_commit" ${GBUILD_FLAGS} "$l_descriptor"
...
g_sigs_dir=${nainaide_SIGS:-"${g_workdir}/${DEFAULT_NAINAIDE_SIGS}"}
