package app

import (
	"encoding/hex"
	"github.com/FnyaMing/nainaide/x/auth"
	"os"
	"testing"

	"github.com/stretchr/testify/require"
	"github.com/tendermint/tendermint/libs/log"
	"github.com/tendermint/tm-db"

	"github.com/FnyaMing/nainaide/codec"
	"github.com/FnyaMing/nainaide/simapp"
	abci "github.com/tendermint/tendermint/abci/types"
)

func TestnainaidedExport(t *testing.T) {
	db := db.NewMemDB()
	gapp := NewnainaideApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, 0)
	setGenesis(gapp)

	// Making a new app object with the db, so that initchain hasn't been called
	newGapp := NewnainaideApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, 0)
	_, _, err := newGapp.ExportAppStateAndValidators(false, []string{})
	require.NoError(t, err, "ExportAppStateAndValidators should not have an error")
}

func TestTxDecoder(t *testing.T) {
	cdc := MakeCodec()
	//txBytes, _ := hex.DecodeString("")
	txBytes, _ := hex.DecodeString("")
	decoder := auth.DefaultTxDecoder(cdc)
	tx, _ := decoder(txBytes)
	msg := tx.GetMsgs()
	t.Log(len(msg))
}

func setGenesis(gapp *nainaideApp) error {

	genesisState := simapp.NewDefaultGenesisState()
	stateBytes, err := codec.MarshalJSONIndent(gapp.cdc, genesisState)
	if err != nil {
		return err
	}

	// Initialize the chain
	gapp.InitChain(
		abci.RequestInitChain{
			Validators:    []abci.ValidatorUpdate{},
			AppStateBytes: stateBytes,
		},
	)
	gapp.Commit()
	return nil
}
