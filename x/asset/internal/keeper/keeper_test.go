...
func TestSendKeeper(t *testing.T) {
	...
	token := types.NewToken("btc", "bitcoin", 6, 21000000000000, false, "bitcoin on nainaidenet", addr1)
	...
	token = types.NewToken("eth", "ethereum", 6, 100000000000000, true, "ethereum on nainaidenet", addr1)
	keeper.SetToken(ctx, token)
	require.True(t, keeper.IsTokenExist(ctx, "eth"))

	token = types.NewToken("eth", "ethereum", 6, 110000000000000, true, "ethereum on nainaidenet", addr1)
